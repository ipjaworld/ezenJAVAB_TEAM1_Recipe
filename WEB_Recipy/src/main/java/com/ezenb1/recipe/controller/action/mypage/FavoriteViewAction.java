...
public class FavoriteViewAction implements Action {

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		String url="mypage/favorite.jsp";
		...
		//	InterestDao idao = InterestDao.getInstance();
			int fnum = rdao.insertfavoriteRecipe(request.getParameter("rnum"), mvo.getId() );  // 주문 내역와 아이디를 보냄(odao~)
			
			// 방금 주문에 성공한 주문 번호를 갖고 오더 리스트로 이동하여 주문번호로 주문 내역을 다시 조횐하고 jsp로 이동합니다.
			url ="recipe.do?command=favoriteView&fnum="+fnum;
