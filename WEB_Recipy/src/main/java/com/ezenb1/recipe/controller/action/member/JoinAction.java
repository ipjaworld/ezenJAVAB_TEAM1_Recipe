...
public class JoinAction implements Action {
	...
	//	mvo.setIndate(Timestamp.valueOf(multi.getParameter("indate")));
		...
		if(result==1) {
			session.setAttribute("message", "회원가입 완료~! 로그인하세요!");
		}else {
			session.setAttribute("message", "회원가입 실패~! 다음에 다시 시도하세요! 계속 실패하면 관리자에게 문의하세요");
		}
		
		response.sendRedirect("recipe.do?command=loginForm");
